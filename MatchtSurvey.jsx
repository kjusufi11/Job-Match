...
// ── SHARED CONSTANTS (keep in sync with MatchtRecruiterSurvey.jsx) ────────────
const INDUSTRIES = [
...
const PERSONALITY_DIMS = [
  ...
  { id:"recognition", q:"I prefer recognition that is...",                  low:"Private — a personal thank-you is enough",high:"Public — acknowledged openly" },
...
const C = {
  ...
  gray800:"#2B3D4D",border:"#D4E3EC",green:"#19A87A",red:"#C0392B",
};
...
// Radio — use for ≤6 options where reading all choices matters
...
// Pills — use for ≤8 multi-select options that are short labels
...
// Searchable dropdown — use for 9+ options
...
function S1({d,set}){return <>
  ...
  <Sub>Fundamentals first. This personalizes your experience and surfaces the right roles.</Sub>
  <Divider/>
...
function S2({d,set}){
  ...
  return <>
    ...
    <FSelect value={d.education} onChange={v=>set(x=>({...x,education:v}))} options={EDUCATION_LEVELS} placeholder="Select your education level..."/>
    <Divider/>
    {hasDegree&&<>
...
function S3({d,set}){
  ...
  const fmtReports=v=>v===0?"None":v>=50?"50+":String(v);
  return <>
    ...
    <Divider/>
    <QLabel required>Years in your primary function</QLabel>
    ...
    <Divider/>
    <QLabel>Max number of direct reports you've managed</QLabel>
    <Sub>Set to 0 if you've never managed people directly.</Sub>
...
function S4({d,set}){return <>
  ...
  <Divider/>
  <QLabel required>Soft skills — your strongest (up to 8)</QLabel>
  <MultiDropdown options={SOFT_SKILLS} values={d.softSkills} onChange={v=>v.length<=8&&set(x=>({...x,softSkills:v}))} placeholder="Search and select soft skills..." max={8}/>
  {d.softSkills.length>0&&<div style={{fontSize:12,color:C.gray400,marginTop:6,fontFamily:F}}>{d.softSkills.length}/8 selected</div>}
  <Divider/>
  <QLabel required>Technical & functional skills</QLabel>
...
function S5({d,set}){
  ...
  return <>
    ...
    <Sub>Your ranges and non-negotiables. Honest answers filter out roles that won't work for you.</Sub>
    <Divider/>
    ...
    <Sub>We only show you roles within this range.</Sub>
    ...
    {!noCommute&&<><Divider/>
      <QLabel>Maximum one-way commute time</QLabel>
...
function S6({d,set}){return <>
  ...
  <Sub>Pick all that resonate. These exact descriptors are what companies use to describe themselves.</Sub>
...
function S7({d,set}){return <>
  ...
  <Sub>Our workplace-calibrated personality assessment. Matched directly against how employers describe what their role requires. No right or wrong answers.</Sub>
  <Divider/>
...
function S8({d,set}){return <>
  ...
  <Sub>Understanding where you're headed helps us find roles that move you forward.</Sub>
  <Divider/>
  ...
  <FTextarea value={d.personalNote} onChange={v=>set(x=>({...x,personalNote:v}))} placeholder="e.g. Relocating to Austin in Q3. Looking for autonomy. Portfolio at..." rows={4}/>
</>;}
...
function Review({data}){
  ...
  const rows=[
    ...
    ["Industries (worked in)",data.industries.slice(0,3).join(", ")+(data.industries.length>3?` +${data.industries.length-3} more`:"")],
    ["Soft skills",data.softSkills.slice(0,3).join(", ")+(data.softSkills.length>3?"...":"")],
    ["Tech skills",data.techSkills.slice(0,3).join(", ")+(data.techSkills.length>3?"...":"")],
  ...
  return <>
    ...
    <div style={{marginTop:22,background:C.tealDim,border:`1px solid ${C.tealBorder}`,borderRadius:9,padding:"14px 16px"}}>
      <p style={{fontSize:13,color:C.teal,fontWeight:600,margin:0,fontFamily:F}}>✓ Your profile is saved. You'll be notified the moment a role matches your criteria.</p>
    </div>
  </>;}
...
const SECTIONS=[
  ...
  {label:"Skills",C:S4},{label:"Job Preferences",C:S5},{label:"Work Style & Culture",C:S6},
...
const INIT={
  ...
  currentTitle:"",currentEmployer:"",totalExp:0,fieldExp:0,longestTenure:0,industries:[],directReports:0,empStatus:"",gaps:"",
    ...
    <div style={{background:C.bg,minHeight:"100vh",fontFamily:F,display:"flex",alignItems:"center",justifyContent:"center",padding:24}}>
      ...
      <div style={{textAlign:"center",maxWidth:400}}>
        ...
        <div style={{background:C.tealDim,border:`1px solid ${C.tealBorder}`,borderRadius:10,padding:"16px 18px"}}>
          <p style={{fontSize:13,color:C.teal,fontWeight:600,margin:0}}>Next: upload a short video intro to boost your visibility by 4×.</p>
        </div>
      </div>
    </div>
  );
